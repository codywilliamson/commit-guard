...
test_installer_supports_native_hooks_and_pr_title_mode_without_node_repo() {
  ...
  assert_contains "created: .commit-guard.yml" "$output" "expected starter config output"
  assert_contains 'pr-mode: title' "$(cat "${repo_dir}/.commit-guard.yml")" "expected pr mode in starter config"
  assert_contains 'ai-attribution: block' "$(cat "${repo_dir}/.commit-guard.yml")" "expected ai-attribution default in starter config"
}
